...
//校时结构参数
	...
	public static class NET_DVR_HANDLEEXCEPTION_V30 extends Structure {
		public int dwHandleType; /* 处理方式,处理方式的"或"结果 *//* 0x00: 无响应 *//* 0x01: 监视器上警告 *//* 0x02: 声音警告 *//* 0x04: 上传中心 */ /*
																														 * 0x08:
																														 * 触发报警输出
																														 *//*
																															 * 0x20:
...
//通道图象结构
	...
	public static class NET_DVR_COMPRESSION_INFO_V30 extends Structure {
		...
		public byte byResolution; // 分辨率0-DCIF 1-CIF, 2-QCIF, 3-4CIF, 4-2CIF 5（保留）16-VGA（640*480）
									// 17-UXGA（1600*1200） 18-SVGA （800*600）19-HD720p（1280*720）20-XVGA 21-HD900p
		public byte byBitrateType; // 码率类型 0:定码率，1:变码率
		public byte byPicQuality; // 图象质量 0-最好 1-次好 2-较好 3-一般 4-较差 5-差
		public int dwVideoBitrate; // 视频码率 0-保留 1-16K 2-32K 3-48k 4-64K 5-80K 6-96K 7-128K 8-160k 9-192K 10-224K
									// 11-256K 12-320K 13-384K 14-448K 15-512K 16-640K 17-768K 18-896K 19-1024K
									// 20-1280K 21-1536K 22-1792K 23-2048最高位(31位)置成1表示是自定义码流, 0-30位表示码流值。
		public int dwVideoFrameRate; // 帧率 0-全部; 1-1/16; 2-1/8; 3-1/4; 4-1/2; 5-1; 6-2; 7-4; 8-6; 9-8; 10-10; 11-12;
										// 12-16; 13-20; V2.0版本中新加14-15; 15-18; 16-22;
	...
	public static class NET_DVR_COMPRESSION_INFO extends Structure {// 码流压缩参数(子结构)
		...
		public byte byPicQuality; // 图象质量 0-最好 1-次好 2-较好 3-一般 4-较差 5-差
		public int dwVideoBitrate; // 视频码率 0-保留 1-16K(保留) 2-32K 3-48k 4-64K 5-80K 6-96K 7-128K 8-160k 9-192K
	...
	public static class NET_DVR_COMPRESSION_INFO_EX extends Structure {// 码流压缩参数(子结构)(扩展) 增加I帧间隔
		...
		// 最高位(31位)置成1表示是自定义码流, 0-30位表示码流值(MIN-32K MAX-8192K)。
		public int dwVideoFrameRate; // 帧率 0-全部; 1-1/16; 2-1/8; 3-1/4; 4-1/2; 5-1; 6-2; 7-4; 8-6; 9-8; 10-10; 11-12;
										// 12-16; 13-20, //V2.0增加14-15, 15-18, 16-22;
		public short wIntervalFrameI; // I帧间隔
...
//云台协议表结构配置
	...
	public static class NET_DVR_SINGLE_RS232 extends Structure {// RS232串口参数配置(9000扩展)
		public int dwBaudRate; /*
								 * 波特率(bps)，0－50，1－75，2－110，3－150，4－300，5－600，6－1200，7－2400，8－4800，9－9600，10－
	...
	public static class NET_DVR_ALARMINFO_V30 extends Structure {// 上传报警信息(9000扩展)
		public int dwAlarmType;/*
								 * 0-信号量报警,1-硬盘满,2-信号丢失,3－移动侦测,4－硬盘未格式化,5-读写硬盘出错,6-遮挡报警,7-制式不匹配, 8-非法访问,
								 * 0xa-GPS定位信息(车载定制)
								 */
		...
		public byte[] byAlarmRelateChannel = new byte[MAX_CHANNUM_V30];/*
																		 * 触发的录像通道，为1表示对应录像,
																		 * dwAlarmRelateChannel[0]对应第1个通道
																		 */
	...
	public static class NET_DVR_ALARMINFO extends Structure {
		public int dwAlarmType;/*
								 * 0-信号量报警,1-硬盘满,2-信号丢失,3－移动侦测,4－硬盘未格式化,5-读写硬盘出错,6-遮挡报警,7-制式不匹配, 8-非法访问, 9-串口状态,
								 * 0xa-GPS定位信息(车载定制)
								 */
	...
	public static class NET_DVR_ALARMINFO_EX extends Structure {// 上传报警信息(杭州竞天定制 2006-07-28)
		...
		public int[] dwAlarmRelateChannel = new int[MAX_CHANNUM];/*
																	 * 报警输入端口对应的录像通道，哪一位为1表示对应哪一路录像,dwAlarmRelateChannel
																	 * [0]对应第1个通道
																	 */
...
//辅助输出参数配置
	public static class NET_DVR_AUXOUTCFG extends Structure {
		...
		public int[] dwAuxSwitchTime = new int[MAX_AUXOUT]; /*
															 * 辅助输出切换时间:
															 * 0-不切换,1-5s,2-10s,3-20s,4-30s,5-60s,6-120s,7-300s
															 */
...
//手动查看 2007-12-28
	public static class NET_DVR_MATRIX_ROW_COLUMN_LINK extends Structure {
		public int dwSize;
		/*
		 * 以下三个参数只需要指定其中一个便可指定数字矩阵里的某一行 所代表的远程监控通道。 如果指定了多个域并有冲突，设备将按照域的先后顺序为准取最先定义者。
		 */
...
//布防参数
	public static class NET_DVR_SETUPALARM_PARAM extends Structure {
		...
		public short wTaskNo; // 任务处理号 和 (上传数据NET_DVR_VEHICLE_RECOG_RESULT中的字段dwTaskNo对应 同时 下发任务结构
...
//穿越警戒面参数
	...
	public static class NET_DVR_HANDLEEXCEPTION_V40 extends Structure {
		public int dwHandleType;/*
								 * 处理方式，各种异常处理方式的"或"结果，异常处理方式： 0x00: 无响应 0x01: 监视器上警告 0x02: 声音警告 0x04: 上传中心
								 * 0x08: 触发报警输出 0x10: Jpeg抓图并上传EMail 0x20: 无线声光报警器联动 0x40: 联动电子地图(目前仅PCNVR支持)
								 * 0x200：抓图并上传ftp 0x400: 虚焦侦测联动聚焦 0x800: PTZ联动跟踪(球机跟踪目标) E.g.
								 * dwHandleType==0x01|0x04 表示配置报警发生时联动监视器上警告并且将报警信息上传中心。
								 */
		...
		public int[] dwRelAlarmOut = new int[MAX_CHANNUM_V30];/*
																 * 触发报警输出通道，取数组前dwRelAlarmOutChanNum个值，
																 * 其值表示报警输出通道号(从1开始)，初始值是0xfffffffff(不关联通道)。
																 * 例如，dwRelAlarmOutChanNum=5，则可以配置触发报警输出通道dwRelAlarmOut[
																 * 0]~dwRelAlarmOut[4]。
																 */
...
//车牌识别结果子结构
	public static class NET_DVR_PLATE_INFO extends Structure {
		...
		public Pointer pXmlBuf; // XML报警信息指针,报警类型为 COMM_ITS_PLATE_RESUL时有效，其XML对应到EventNotificationAlert XML
								// Block
	...
	public static class NET_DVR_VEHICLE_INFO extends Structure {
		...
		/*
		 * 违规类型，0-正常，1-低速，2-超速，3-逆行，4-闯红灯,5-压车道线,6-不按导向，7-路口滞留， 8-机占非，9-违法变道，10-不按车道
		 * 11-违反禁令，12-路口停车，13-绿灯停车, 14-未礼让行人(违法代码1357),
		 * 15-违章停车，16-违章掉头,17-占用应急车道,18-禁右,19-禁左,20-压黄线,21-未系安全带,22-行人闯红灯,23-加塞,24-
		 * 违法使用远光灯， 25-驾驶时拨打接听手持电话，26-左转不让直行，27-右转不让左转，28-掉头不让直行，29-大弯小转,
...
//交通取证报警
	public static class NET_DVR_TFS_ALARM extends Structure {
		...
		public byte byRegion; // 区域索引值,0-保留，1-欧洲(Europe Region)，2-俄语区域(Russian Region)，3-欧洲&俄罗斯(EU&CIS) ,
								// 4-中东（Middle East），0xff-所有
...
interface PlayCtrl extends StdCallLibrary {
	PlayCtrl INSTANCE = (PlayCtrl) Native.loadLibrary("D:\\HCdll\\PlayCtrl.dll", PlayCtrl.class);
